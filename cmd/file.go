...
func generateNewFileSQL(nameFile string, description string, idcobertura string) {
	...
	s := `INSERT INTO seguros.cobertura (idcobertura, tipo, codigo, descripcion, suma, prima, tasa, deducible_mto, 
deducible_porcentaje, deducible_minimo, deducible_base, orden, especificacion, indestandar, idaseguradora, 
indacumulasuma, categoria, subGrupo, id_tarifa, deducible_monto, indeliminable, indcobdecl, estado) 
VALUES (` + idcobertura + `, 'exclusion', '', '` + description + `', 0.00, 0.00, 
0.000000, NULL, 0.000000, 0.00, 'null', 0, '', 'S', 1, 'N', 'DAÑOS', NULL, 0, 0, 'S', NULL, NULL);
...
VALUES (` + idcobertura + `, 10, 'exclusion', NULL, NULL, 0.00, 0.00, 0.000000, 0.00, 0.000000, 0.00, '0', 0, 0, 'N', 'N', 'S', 'N', 'S', 'S',
NULL, 'NO', 'BIENESINCE', NULL, NULL, 'N', 'S', 'N', 'N');

INSERT INTO seguros.plantilla_ramo_cobertura
(idplantilla, version, idramo, idcobertura, descripcion) VALUES (36, 1, 10,` + idcobertura + `, NULL);`

	b := []byte(s)

	err := ioutil.WriteFile("querys/"+nameFile+".sql", b, 0644)
	if err != nil {
		log.Fatal(err)
	}
}
